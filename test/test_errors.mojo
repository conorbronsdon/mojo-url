"""Unit tests for `url.errors` — position-aware parse errors.

`line_col` and `parse_error` follow the error-reporting pattern shared
across the mojo-* parser suite: 1-based line, byte column, LF-only line
terminator, clamped offsets, and a whitespace-trimmed ~30-byte snippet
with `...` truncation markers and UTF-8-safe window edges. Every
documented edge case gets a test. URLs are single-line in practice, so
the multi-line cases exercise the shared semantics rather than a URL
code path.
"""

from std.testing import assert_equal, assert_true, TestSuite

from url import line_col, parse_error
...
def test_parse_error_long_url_column_points_at_offending_byte() raises:
    # URL-shaped single-line source: the offending '%' of a truncated
    # escape deep in a long query string still gets a precise column and
    # a windowed snippet.
    var source = String("https://example.com/search?q=")
    for _ in range(30):
        source += "a"
    source += "%2"  # truncated escape: '%' at byte 59, column 60
    var e = parse_error("truncated percent-escape", source.as_bytes(), 59)
    var rendered = _msg(e)
    assert_true(
        rendered.startswith("truncated percent-escape at line 1, column 60:")
    )
    assert_true("%2'" in rendered)
